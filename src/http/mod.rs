...
impl HttpClient {
    ...
    pub async fn get_network_status(&self) -> HttpResult<HttpModemNetworkStatusResponse> {
        self.modem_request("/sms/modem-status", "NetworkStatus").await
    }
    ...
    /// Get the Modem Hat's battery level, which is used for GNSS module warm starts.
    pub async fn get_battery_level(&self) -> HttpResult<HttpModemBatteryLevelResponse> {
        self.modem_request("/sms/battery-level", "BatteryLevel").await
    }

    /// Get GNSS module fix status.
    pub async fn get_gnss_status(&self) -> HttpResult<FixStatus> {
        self.modem_request("/gnss/status", "GNSSStatus").await
    }

    /// Get a PositionReport from GNSS module.
