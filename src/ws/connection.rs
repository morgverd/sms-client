...
impl ConnectionParams {
    ...
    pub fn from_config(
        ...
        let mut url = config.url.parse::<url::Url>()
    ...
    pub async fn connect(&self) -> WebsocketResult<tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>> {
        ...
        if let Some(token) = &self.authorization {
            request.headers_mut()
                .append("authorization", token.clone());
        }
        ...
        let ws_stream = match &self.connector {
            ...
            Some(connector) => {
                match tokio_tungstenite::connect_async_tls_with_config(
                    request, None, false, Some(connector.clone()),
                ).await {
                    Ok((stream, _)) => stream,
    ...
    ) -> WebsocketResult<tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>> {
