...
sms-client = { version = "2.0.0", features = ["websocket"] }

# Includes ONLY the WebSocketClient.
sms-client = { version = "2.0.0", default-features = false, features = ["websocket"] }

# Includes BOTH, with Rust-TLS.
sms-client = { version = "2.0.0", features = ["http-tls-rustls", "websocket-tls-rustls"] }

# Includes BOTH, with native TLS.
sms-client = { version = "2.0.0", features = ["http-tls-native", "websocket-tls-native"] }
```
    ...
    client
        .on_message(move |message, client| {
            ...
            match message {
                ...
                Event::OutgoingMessage(sms) => {
                    log::info!("Outgoing Message: {sms:?}")
                }
                Event::DeliveryReport { message_id, report } => {
                    log::info!("Received delivery report for #{message_id}: {report:?}")
                }
                Event::ModemStatusUpdate { current, previous } => {
                    log::info!("Modem HAT status {current} -> {previous}")
                }
                Event::GnssPositionReport(position) => {
                    log::info!("GNSS Position report: {position:?}")
                }

                // Generated by the client for websocket connection changes.
