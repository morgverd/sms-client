...
sms-client = { version = "2.1.0", features = ["websocket"] }

# Includes ONLY the WebSocketClient.
sms-client = { version = "2.1.0", default-features = false, features = ["websocket"] }

# Includes BOTH, with Rust-TLS.
sms-client = { version = "2.1.0", features = ["http-tls-rustls", "websocket-tls-rustls"] }

# Includes BOTH, with native TLS.
sms-client = { version = "2.1.0", features = ["http-tls-native", "websocket-tls-native"] }
```
...
Read over all messages from a phone_number with, with a default chunk size of 50.
...
async fn read_all_messages(phone_number: &str, http: Arc<HttpClient>) {
    ...
    // Iterate over every message associated with phone_number.
